...
* Start iteration over outcomes
	...
	foreach out in "infsurvival" "conception" "livebirth" "concep_restr" {  
		...
		* Choose datafile based on outcome, set macros 
		if "`outcome'" == "conception" {
			...
			local note "Table presents results from quadratic models examining the effect of dominance rank (left three columns) and rainfall (right three columns) on the probability of conceiving, given that a female was cycling at the start of a given observation month. `notecommon'"
		} 
		if "`outcome'" == "concep_restr" {
			local datafile "conception_data_restricted_4.16.2024" // if restricted to parous who successfully weaned their prior infant	
			local ytitle "Conception probability"
			local caption "the probability of conceiving, given that a female was cycling at the start of the month"	
			local note "Notes. Table presents results from quadratic models examining the effect of dominance rank (left three columns) and rainfall (right three columns) on the probability of conceiving, given that a female was cycling at the start of a given observation month. `notecommon'"
		} 
		if "`outcome'" == "livebirth" {
			...
			local note "Table presents results from quadratic models examining the effect of dominance rank (left three columns) and rainfall (right three columns) on the probability of giving birth to a live infant, given that a female was pregnant. `notecommon'"
		} 
		if "`outcome'" == "infsurvival" {
			...
			local note "Table presents results from quadratic models examining the effect of dominance rank (left three columns) and rainfall (right three columns) on the probability of successfully raising an infant to 70 weeks (the average age at weaning), given that a female gave birth to a live infant. `notecommon'"
		} 
...
* Start iteration over environments
	...
	** Assign outcome and environment variables
		...
		gen difpos = dif if dif >= 0
		gen difneg = dif if dif < 0
	...
	* Binscatter
		...
		// Only generate for infant survival and rank.  
	...
	* Export results to latex file
		...
		esttab using "$overleaftabdir/tab_`out'_input.tex", ///
			...
			mgroups("Rank" "Rain", pattern(1 0 0 1 0 0) ///
